...
async function translateLyrics(lyrics, songName, artist) {
  ...
  const prompt = `Translate the lyrics of "${songName}" by "${artist}" into natural Japanese.

- Translation should feel natural as a song (not word-for-word literal)
- Preserve emotion, nuance, and poetic feel
- Mirror the structure of the original as closely as possible
...
${lyricsSection}
${feedbackSection}
Create a natural 4-5 turn conversation between two people.
...
async function generateSong(songName, artist, songsDir, { onStatus } = {}) {
  ...
  const ytData = await fetchYouTubeData(songName, artist);
  ...
  for (let i = 0; i < processedTurns.length; i++) {
    ...
    song.conversation.push({
      ...
      // Strip any markers Gemini accidentally added to the Japanese translation
