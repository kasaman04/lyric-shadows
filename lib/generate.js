...
async function callGemini(prompt, retries = 5) {
  for (let attempt = 1; attempt <= retries; attempt++) {
    ...
    try {
      ...
      // Retryable errors: 429 (rate limit) and 503 (high demand)
      const isRetryable = status === 429 || status === 503 || errMsg.includes('high demand');

      if (isRetryable) {
        // Try switching to backup key on 429
        if (status === 429 && activeGeminiKey === GEMINI_API_KEY && GEMINI_API_KEY_BACKUP) {
          console.log('  🔄 Switching to backup Gemini API key...');
          activeGeminiKey = GEMINI_API_KEY_BACKUP;
          continue;
        }
        if (attempt < retries) {
          const waitSec = status === 503 ? 15 : (() => {
            const m = errMsg.match(/retry in ([\d.]+)s/i);
            return m ? Math.ceil(parseFloat(m[1])) + 5 : 30;
          })();
          console.log(`  ⏳ ${status === 503 ? 'High demand' : 'Rate limit'}. Waiting ${waitSec}s (retry ${attempt}/${retries - 1})...`);
...
Translate the lyrics into natural Japanese for a language learning app.

CRITICAL RULES:
- IMPORTANT: You MUST output exactly ONE JSON array element per single line of the original lyrics.
- DO NOT group multiple lines together into a single element.
- If a line is empty (e.g. paragraph break or section header like "[Verse 1]"), output an element with the exact line text in "en" and an empty string "" in "ja".
- Each line's translation should map specifically to that line where possible, while keeping the overall poetic flow natural.
- Do NOT output markdown outside of the JSON array.
...
Return ONLY a valid JSON array matching line-by-line exactly. Example:
[
  { "en": "[Verse 1]", "ja": "" },
  { "en": "First line of the song", "ja": "曲の最初の一行" },
  { "en": "Second line of the song", "ja": "曲の二行目" },
  { "en": "", "ja": "" }
]`;
    try {
  ...
  return finalFixed.length > 0 ? finalFixed : null;
}
...
async function generateConversation(songName, artist, lyrics, { onStatus, contextHint } = {}) {
  ...
  for (let attempt = 1; attempt <= MAX_ATTEMPTS; attempt++) {
    ...
    const prompt = buildConversationPrompt(songName, artist, lyrics, feedback, contextHint);
    const text = await callGemini(prompt);
